...
UpsettiObj <- R6Class("Upsetti",
                       public = list(
                         JobName = NULL,
                         groupUnique = 2, #default is at two
                         groupTotal = 2,
                         groupNames = list(),
                         PathToFile = NULL,
                         InputType = "Default", #InputType will be used for ABS or not ABS for meta analysis or just PCA
                         InputDF = data.frame(),
                         ...
                         initialize = function(JobName, PathToFile, InputType = NULL) {
                           ...
                           if(!is.null(InputType)) {self$InputType <- InputType} #only initialized if in input, otherwise "default"
                         },
